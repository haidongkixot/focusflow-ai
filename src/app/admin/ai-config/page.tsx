...
export default function AIConfigPage() {
  ...
  if (loading) return <div className="text-text-secondary">Loading AI configurations...</div>

  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold">AI Configuration</h1>
        <p className="text-text-secondary text-sm mt-1">Manage AI generation settings per content type</p>
      </div>

      {configs.length === 0 && (
        <div className="card text-center py-12">
          <div className="text-5xl mb-4">🤖</div>
          <h2 className="text-xl font-bold mb-2">No AI Configs Yet</h2>
          <p className="text-text-secondary max-w-sm mx-auto">Run the seed script to create default AI configurations for each content type.</p>
        </div>
      )}

      <div className="space-y-6">
        {configs.map(config => (
          <div key={config.id} className="card p-6 space-y-5">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-lg font-semibold">{config.contentType.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}</h3>
                <p className="text-xs text-text-muted font-mono">{config.contentType}</p>
              </div>
              <label className="flex items-center gap-2 cursor-pointer">
                <span className="text-sm text-text-secondary">{config.isActive ? 'Active' : 'Disabled'}</span>
                <div
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <select
                  ...
                  className="w-full bg-bg-elevated border border-gray-700 rounded-lg px-3 py-2 text-text-primary text-sm focus:outline-none focus:border-accent-400"
                >
                  {MODELS.map(m => <option key={m} value={m}>{m}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-text-secondary mb-1">Temperature: {config.temperature.toFixed(1)}</label>
                <input
                  type="range"
                  min="0"
                  max="2"
                ...
                />
                <div className="flex justify-between text-xs text-text-muted"><span>0</span><span>1</span><span>2</span></div>
              </div>
              <div>
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full bg-bg-elevated border border-gray-700 rounded-lg px-3 py-2 text-text-primary text-sm focus:outline-none focus:border-accent-400 resize-y"
              />
            </div>
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button
                ...
                className="px-5 py-2 bg-accent-500 hover:bg-accent-400 text-white rounded-lg text-sm font-medium disabled:opacity-50 transition-colors"
              >
