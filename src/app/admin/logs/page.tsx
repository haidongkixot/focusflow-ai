...
export default async function AdminLogsPage() {
  // Try AuditLog or fall back to recent user activity
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-text-secondary text-sm mt-1">System notifications and activity (last 100)</p>
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-800">
            {notifications.length === 0 ? (
              <tr><td colSpan={5} className="px-4 py-8 text-center text-text-muted">No activity recorded yet</td></tr>
            ) : notifications.map(n => (
              <tr key={n.id} className="hover:bg-bg-elevated/50 transition-colors">
                ...
                <td className="px-4 py-3"><span className={`px-2 py-0.5 rounded-full text-xs ${n.read ? 'text-text-muted' : 'bg-accent-400/20 text-accent-400'}`}>{n.read ? 'read' : 'unread'}</span></td>
